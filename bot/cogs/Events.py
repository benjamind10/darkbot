...
class Events(commands.Cog):
    ...
    async def event_details(self, ctx, event_id: str):
        ...
        try:
            ...
            embed.add_field(
                name="Status", value=status_map.get(event.status, "Unknown"), inline=True
            )
            ...
            embed.add_field(
                name="🕒 Starts", value=f"{start_time_str}\n({relative_str})", inline=False
            )
            ...
            embed.add_field(
                name="Type", value=entity_type_map.get(event.entity_type, "Unknown"), inline=True
            )
            ...
            # Event URL
            embed.add_field(
                name="🔗 Event Link", value=f"[Click to view in Discord]({event.url})", inline=False
            )
    ...
    async def event_users(self, ctx, event_id: str):
        ...
        try:
            ...
            user_list = [
                user.mention for user in users[:50]
            ]  # Limit to 50 to avoid embed size issues
            ...
            for i in range(0, len(user_list), chunk_size):
                ...
                field_name = (
                    f"Users {i + 1}-{min(i + chunk_size, len(user_list))}"
                    if len(user_list) > chunk_size
                    else "Users"
                )
    ...
    async def next_event(self, ctx):
        ...
        try:
            ...
            embed.add_field(
                name="🕒 When", value=f"{start_time_str}\n({relative_str})", inline=False
            )
            ...
            if next_event.user_count:
                embed.add_field(
                    name="👥 Interested", value=f"{next_event.user_count} users", inline=True
                )

            if next_event.cover_image:
                embed.set_thumbnail(url=next_event.cover_image)

            embed.add_field(name="🔗 Link", value=f"[View Event]({next_event.url})", inline=False)
