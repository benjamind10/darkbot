...
class Utility(commands.Cog):
    ...
    async def ip_lookup(self, ctx, ip):
        ...
        try:
            ...
            embed.set_footer(
                text="— Note: Locations and Latitude & Longitude may not be 100% accurate."
            )
    ...
    async def random_color(self, ctx):
        ...
        embed = discord.Embed(
            color=discord.Color(int(f"0x{hex_color}", 16)), title="→ Random Color"
        )
        ...
        embed.add_field(
            name="• CMYK value:", inline=True, value=f"`{rgb_to_cmyk(rgb[0], rgb[1], rgb[2])}`"
        )
        embed.add_field(
            name="• HSV value:", inline=True, value=f"`{rgb_to_hsv(rgb[0], rgb[1], rgb[2])}`"
        )
        embed.add_field(
            name="• HSL value:", inline=True, value=f"`{rgb_to_hsl(rgb[0], rgb[1], rgb[2])}`"
        )
    ...
    async def remind(self, ctx, time, time_measurement, *, reminder):
        ...
        self.logger.info(
            f"Utility | Sent Remind: {ctx.author} | Time: {time} | Unit: {time_unit} | Reminder: {reminder}"
        )
